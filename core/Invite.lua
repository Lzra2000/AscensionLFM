...
-- { name=, role=, checkAt= } - InviteUnit not erroring is NOT reliable
-- proof the target actually joined (they can go offline/disconnect between
-- whispering and the invite firing); verified against roster a few
-- seconds later, same "don't trust a no-error call" pattern Kick.lua uses.
local pendingInviteVerify = {}
local INVITE_VERIFY_DELAY = 5
...
--
-- Prefers C_Manastorm.IsInManastorm via AscensionLFM.API (extract-verified,
-- Safe-wrapped). Falls back to IsInInstance when C_Manastorm is absent.
local function IsHostInsideInstance()
    ...
    -- Fallback when AscensionAPI.lua is missing from a partial install.
...
-- Cooldown alone isn't enough to stop a duplicate invite: an applicant
-- auto-invited via public LFG scan, then whispering the host directly a
-- few seconds later (past the cooldown) unaware they already joined, would
-- otherwise get InviteUnit fired on them a second time. Check roster
-- membership directly instead of trusting the cooldown window.
local function IsAlreadyInGroup(name)
    ...
    if raid > 0 then
        for i = 1, raid do
            local n = type(UnitName) == "function" and UnitName("raid" .. i)
    ...
    for i = 1, party do
        local n = type(UnitName) == "function" and UnitName("party" .. i)
        if n and LowerName(n) == key then
            return true
        end
    end
...
function Invite.InvitePlayer(name, role, hasAura)
    ...
    if role and AscensionLFM.Slots and AscensionLFM.Slots.Assign then
        ...
        table.insert(pendingInviteVerify, {
            name = name,
            role = role,
            checkAt = Now() + INVITE_VERIFY_DELAY,
        })
    end
    if role then
        lastKnownRoles[LowerName(name)] = role
...
local function ScheduleRetry(kind, sender, message, now, previousAttempts)
    ...
    for _, pending in ipairs(pendingRetries) do
        if pending.kind == kind and LowerName(pending.sender) == key then
            -- Applicant whispered again before the queued retry fired -
            -- replay their latest message, not the stale first one.
...
function Invite.Tick(now)
    ...
    local vi = 1
    while vi <= #pendingInviteVerify do
        local v = pendingInviteVerify[vi]
        if now >= (tonumber(v.checkAt) or 0) then
            table.remove(pendingInviteVerify, vi)
            if not IsAlreadyInGroup(v.name) then
                -- Invite never actually landed (target offline/disconnected
                -- etc) - the slot was never really taken, so free it back up
                -- instead of leaving it permanently marked filled.
                if AscensionLFM.Slots and AscensionLFM.Slots.ClearName then
                    AscensionLFM.Slots.ClearName(v.name)
                end
                if AscensionLFM.MainWindow and AscensionLFM.MainWindow.RefreshSlots then
                ...
                if AscensionLFM.Print then
                    AscensionLFM.Print("invite to " .. tostring(v.name) .. " never landed - slot freed")
                end
            end
        else
            vi = vi + 1
