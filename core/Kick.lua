...
local function DoKick(name)
    ...
    -- Try roster name first (may be Name-Realm), then bare name. A pcall
    -- succeeding (no Lua error) is not proof UninviteUnit actually did
    -- anything - it can silently no-op for an unresolved identifier - so
    -- every candidate form is attempted rather than stopping at the first
    -- non-erroring call; CheckVerify() confirms afterward whether any of
    ...
    local anyOk = false
    local lastErr = "UninviteUnit failed"
