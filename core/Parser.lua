...
local ROLE_PATTERNS = {
    tank = { "tanks?", "tnks?", "%f[%w]ot%f[%W]", "%f[%w]mt%f[%W]", "%f[%w]ot$", "%f[%w]mt$" },
    healer = {
        "healers?",
        "heals?",
        "heal",
...
--- Extract the keystone level from an optional "[Dungeon +Level]" tag
-- (the exact format Poster.MPlusPrefix produces - other hosts running
-- this same addon, or typing the convention manually, tag their posts
-- the same way). Returns a number, or nil if the message has no such tag.
function Parser.ExtractKeystoneLevel(message)
