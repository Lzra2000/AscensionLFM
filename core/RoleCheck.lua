...
-- Shared reject-list for raid banter that only talks ABOUT a role rather
-- than declaring one ("nobody offered so ill heal", "does anyone have
-- aura?"). Single source of truth for both ParseBareRoleWord's own
-- substring match AND HandleWhisper's post-hoc filter on ParseWhisperRole
-- results (which otherwise runs no banter filtering at all) - previously
-- these were two separately-maintained lists that had drifted apart.
local function IsRoleBanterText(t)
    return t:find("anyone", 1, true) or t:find("someone", 1, true)
        ...
        or t:find("mean", 1, true) or t:find("why", 1, true)
end
